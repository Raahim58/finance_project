...
class InstrumentAlias(Base):
    ...
    __table_args__ = (
        UniqueConstraint("provider", "alias", "valid_from", name="uq_instrument_alias_validity"),
    )
...
class IngestionCoverage(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "period_key",
            "source",
            name="uq_ingestion_coverage_key",
        ),
    )
...
class MarketObservation(Base):
    ...
    artifact_id: Mapped[str] = mapped_column(
        ForeignKey("source_artifacts.id"), index=True, nullable=False
    )
...
class PortfolioIPS(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    portfolio_id: Mapped[str] = mapped_column(
        ForeignKey("portfolios.id"), unique=True, index=True, nullable=False
    )
...
class PortfolioCashAccount(Base):
    ...
    __table_args__ = (
        UniqueConstraint("portfolio_id", "currency", "name", name="uq_portfolio_cash_account"),
    )
...
class PortfolioPositionSnapshot(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "portfolio_id", "instrument_id", "snapshot_date", name="uq_position_snapshot"
        ),
    )
...
class PortfolioCashSnapshot(Base):
    ...
    __table_args__ = (
        UniqueConstraint("cash_account_id", "snapshot_date", name="uq_cash_snapshot"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    cash_account_id: Mapped[str] = mapped_column(
        ForeignKey("portfolio_cash_accounts.id"), index=True, nullable=False
    )
    snapshot_date: Mapped[date] = mapped_column(Date, index=True, nullable=False)
...
class OptimizerRun(Base):
    ...
    ips_version_id: Mapped[str | None] = mapped_column(
        ForeignKey("portfolio_ips_versions.id"), index=True
    )
...
class OptimizerAllocation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("optimizer_run_id", "instrument_id", name="uq_optimizer_instrument"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    optimizer_run_id: Mapped[str] = mapped_column(
        ForeignKey("optimizer_runs.id"), index=True, nullable=False
    )
    instrument_id: Mapped[str] = mapped_column(
        ForeignKey("instruments.id"), index=True, nullable=False
    )
...
class ScenarioRun(Base):
    ...
    scenario_definition_id: Mapped[str | None] = mapped_column(
        ForeignKey("scenario_definitions.id"), index=True
    )
...
class ScenarioShock(Base):
    ...
    scenario_definition_id: Mapped[str] = mapped_column(
        ForeignKey("scenario_definitions.id"), index=True, nullable=False
    )
...
class MacroSeries(Base):
    ...
    source_id: Mapped[str] = mapped_column(
        ForeignKey("data_sources.id"), index=True, nullable=False
    )
    metadata_json: Mapped[str] = mapped_column(Text, default="{}", nullable=False)
...
class MacroSeriesProvider(Base):
    ...
    series_id: Mapped[str] = mapped_column(
        ForeignKey("macro_series.id"), index=True, nullable=False
    )
...
class StandardizedFinancialFact(Base):
    ...
    classification: Mapped[str] = mapped_column(
        String(40), default="standardized_secondary", nullable=False
    )
...
class CompanyScreeningSnapshot(Base):
    ...
    __table_args__ = (
        UniqueConstraint("instrument_id", "as_of_date", name="uq_company_screening_date"),
    )
...
class NormalizedEventEvidence(Base):
    ...
    normalized_event_id: Mapped[str] = mapped_column(
        ForeignKey("normalized_events.id"), nullable=False, index=True
    )
...
class AssistantMessage(Base):
    ...
    context_receipt_id: Mapped[str | None] = mapped_column(
        ForeignKey("intelligence_context_receipts.id"), index=True
    )
