...
def _seed_company_research(db) -> dict[str, int]:
    ...
    for index, instrument in enumerate(instruments):
        ...
        for year in (2024, 2025):
            title = f"{instrument.symbol} synthetic demo annual facts {year}"
    ...
    with SessionLocal() as db:
        ...
        if selected_constraints.get("notes") != f"Synthetic demo seed {DEMO_SEED_VERSION}":
            save_ips_version(db, user, portfolio.id, IPSDraft(
                ...
                constraints={"long_only": True, "max_instrument_weight": 0.30, "max_sector_weight": 0.40, "min_cash_weight": 0.10, "target_volatility": 0.18, "target_beta": 1.0, "risk_budgets": {"MEBL": 0.14, "SYS": 0.14, "OGDC": 0.12, "FFC": 0.12, "LUCK": 0.10, "HUBC": 0.10, "ILP": 0.09, "MARI": 0.09, "CASH": 0.10}, "notes": f"Synthetic demo seed {DEMO_SEED_VERSION}"},
            ), confirm=True)
        ...
        existing_scenarios = set(db.scalars(select(ScenarioRun.name).where(ScenarioRun.portfolio_id == portfolio.id)))
        for payload in scenario_payloads:
            if payload.name not in existing_scenarios:
                run_scenario(db, user, portfolio.id, payload)
        monitoring_rules = {
            "concentration": {"maximum": 0.30},
            "stale_data": {},
            "drawdown": {"maximum": 0.50},
            "volatility": {"maximum": 0.50},
            "var": {"maximum": 0.20},
            "liquidity": {"minimum_daily_volume": 1, "minimum_traded_value": 1},
            "event": {"lookback_hours": 720},
            "ingestion_failure": {"lookback_hours": 24},
        }
        existing_rules = set(db.scalars(select(MonitoringRule.rule_type).where(MonitoringRule.portfolio_id == portfolio.id)))
