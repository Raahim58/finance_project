...
class ContextBuilder:
    ...
    def __init__(self, *, cache: ContextSectionCache | None = None, now: Callable[[], datetime] | None = None) -> None:
        self.cache = cache or ContextSectionCache()
        self.now = now or (lambda: datetime.now(UTC))

    def build(self, db: Session, user: User, request: IntelligenceContextRequest) -> IntelligenceContext:
        build_started = perf_counter()
        built_at = self.now()
        instrument = db.scalar(select(Instrument).where(func.upper(Instrument.symbol) == request.symbol.upper()))
        if instrument is None:
            raise HTTPException(status_code=404, detail="Instrument not found")
        portfolio, ips = self._validate_personalized_scope(db, user, request)
        context_id = f"ctx_{_hash(CONTEXT_CONTRACT_VERSION, user.id, request.model_dump(mode='json'))[:24]}"
        ...
        evidence_ids = sorted({item.evidence_id for section in sections.values() for item in section.evidence})
        ...
        content_hash = _hash(
            ...
            {name: section.model_dump(exclude={"reused"}, mode="json") for name, section in sections.items()},
        ...
        degraded = any(state not in {ContextState.CURRENT, ContextState.NOT_REQUESTED} for state in states.values())
        return IntelligenceContext(
            ...
            message="Some requested data is unavailable or not current; refresh may be requested." if degraded else None,
            sections=sections,
    ...
    def _validate_personalized_scope(
        ...
        ips = db.get(PortfolioIPSVersion, portfolio.selected_ips_version_id) if portfolio.selected_ips_version_id else None
        if request.scope == ContextScope.SECURITY_FIT and (ips is None or ips.status != "confirmed"):
            raise HTTPException(status_code=422, detail="Security Fit requires the selected portfolio's confirmed IPS")
    ...
    def _portfolio(db: Session, user: User, instrument: Instrument, portfolio: Portfolio | None):
        ...
        evidence = [_evidence("calculation", f"portfolio:{portfolio.id}:{portfolio.updated_at.isoformat()}", "portfolio_valuation", as_of=summary.data_freshness_date)]
        state = ContextState.CURRENT if summary.valuation_complete else ContextState.INCOMPLETE
        missing = [] if summary.valuation_complete else [_deficiency(
            instrument,
            "portfolio_valuation",
        ...
        return _section(ContextSectionName.PORTFOLIO, state, data, evidence, as_of=summary.data_freshness_date), missing

    @staticmethod
    def _ips(instrument: Instrument, ips: PortfolioIPSVersion | None):
        if ips is None or ips.status != "confirmed":
            return _section(ContextSectionName.IPS, ContextState.MISSING, None, []), [_deficiency(
                instrument,
                "confirmed_ips",
        ...
        evidence = [_evidence("mandate", f"ips:{ips.id}:v{ips.version}", "confirmed_portfolio_ips", as_of=ips.confirmed_at)]
        return _section(ContextSectionName.IPS, ContextState.CURRENT, data, evidence, as_of=ips.confirmed_at), []

    @staticmethod
    def _company_facts(db: Session, _user: User, _request: IntelligenceContextRequest, instrument: Instrument):
        filing = list(db.scalars(select(FinancialFact).where(FinancialFact.instrument_id == instrument.id).order_by(FinancialFact.period_end.desc(), FinancialFact.version.desc()).limit(100)))
        standardized = list(db.scalars(select(StandardizedFinancialFact).where(StandardizedFinancialFact.instrument_id == instrument.id, StandardizedFinancialFact.quality_status == "observed").order_by(StandardizedFinancialFact.period_end.desc()).limit(100)))
        facts = [
            {"id": row.id, "metric": row.taxonomy_key, "period_type": row.period_type, "period_end": row.period_end, "filing_date": row.filing_date, "value": row.value, "unit": row.unit, "currency": row.currency, "document_id": row.document_id, "version": row.version}
            for row in filing
        ] + [
            {"id": row.id, "metric": row.metric, "period_type": row.period_type, "period_end": row.period_end, "value": row.value, "unit": row.unit, "currency": row.currency, "source_url": row.source_url, "classification": row.classification}
            for row in standardized
        ]
        evidence = [
            _evidence("structured_fact", f"financial_fact:{row.id}:v{row.version}", row.source_label or "financial_facts", as_of=row.filing_date or row.period_end, metadata={"document_id": row.document_id, "page_number": row.page_number})
            for row in filing
        ] + [
            _evidence(
                "structured_fact",
        ...
        data = {"instrument": {"id": instrument.id, "symbol": instrument.symbol, "name": instrument.name, "sector": instrument.sector, "currency": instrument.currency, "type": instrument.instrument_type}, "fundamentals": facts}
        if facts:
            as_of = max((row.get("filing_date") or row["period_end"] for row in facts), default=None)
            return _section(ContextSectionName.COMPANY_FACTS, ContextState.CURRENT, data, evidence, as_of=as_of, provenance={"exact_values": "structured_database_only"}), []
        missing = [_deficiency(instrument, "financial_facts", ContextState.MISSING, "No observed structured financial facts are available.", {"dataset": "financial_facts", "cadence": "reporting_period"})]
        return _section(ContextSectionName.COMPANY_FACTS, ContextState.INCOMPLETE, data, evidence, provenance={"exact_values": "structured_database_only"}), missing

    def _market_risk(self, db: Session, _user: User, _request: IntelligenceContextRequest, instrument: Instrument):
        price = latest_price(db, instrument.symbol)
        snapshot = db.scalar(select(CompanyScreeningSnapshot).where(CompanyScreeningSnapshot.instrument_id == instrument.id).order_by(CompanyScreeningSnapshot.as_of_date.desc()).limit(1))
        if price is None:
            return _section(ContextSectionName.MARKET_RISK, ContextState.MISSING, None, []), [_deficiency(instrument, "market_price", ContextState.MISSING, "No selected canonical market observation is available.", {"dataset": "daily_market", "cadence": "trading_session"}, urgency="high")]
        ...
        now = self.now()
        intervening_sessions = sessions_between(
            db, price.trade_date + timedelta(days=1), now.date()
        ) if price.trade_date < now.date() else []
        expected = {
        ...
        data = {"price": {"trade_date": price.trade_date, "close": price.close, "previous_close": price.previous_close, "change_percent": price.change_percent, "volume": price.volume, "market_cap": price.market_cap, "adjustment_state": price.adjustment_state, "quality_status": price.quality_status}, "risk_metrics": json.loads(snapshot.metrics_json) if snapshot else None, "risk_calculation_as_of": snapshot.as_of_date if snapshot else None}
        market_version = price.artifact_id or _hash(
            price.trade_date, price.close, price.previous_close, price.volume, price.observed_at
        )[:16]
        evidence = [_evidence("market_observation", f"market:{market_version}:symbol:{instrument.symbol}:date:{price.trade_date}", price.source, as_of=price.observed_at or price.trade_date, source_url=price.source_url)]
        if snapshot:
            evidence.append(_evidence("calculation", f"screening:{snapshot.id}:{snapshot.computed_at.isoformat()}", "company_screening", as_of=snapshot.as_of_date))
        missing = [] if state == ContextState.CURRENT else [_deficiency(instrument, "market_price", state, "The authoritative market source is outside its own freshness SLA.", expected, urgency="high")]
        if snapshot is None:
            missing.append(_deficiency(instrument, "company_risk_metrics", ContextState.NOT_EVALUATED, "No deterministic company risk calculation is available.", {"dataset": "company_screening", "dependency": "market_history"}))
            if state == ContextState.CURRENT:
                state = ContextState.INCOMPLETE
        return _section(ContextSectionName.MARKET_RISK, state, data, evidence, as_of=price.trade_date, provenance={"freshness_policy": expected}), missing

    @staticmethod
    def _sector(db: Session, _user: User, _request: IntelligenceContextRequest, instrument: Instrument):
        rows = get_sectors(db)
        selected = next((row for row in rows if row.sector.lower() == (instrument.sector or "").lower()), None)
        if selected is None:
            return _section(ContextSectionName.SECTOR, ContextState.MISSING, None, []), [_deficiency(instrument, "sector_context", ContextState.MISSING, "No canonical sector comparison is available.", {"sector": instrument.sector, "cadence": "trading_session"})]
        data = {"company_sector": selected.model_dump(), "comparisons": [row.model_dump() for row in rows[:20]]}
        evidence = [_evidence(
            "structured_fact",
        ...
        return _section(ContextSectionName.SECTOR, ContextState.CURRENT, data, evidence, as_of=selected.trade_date), []

    @staticmethod
    def _macro(db: Session, user: User, _request: IntelligenceContextRequest, instrument: Instrument):
    ...
    @staticmethod
    def _events(db: Session, _user: User, request: IntelligenceContextRequest, instrument: Instrument):
        events = list_normalized_events(db, subject_type="instrument", subject_key=instrument.symbol, view="company_relevant", limit=request.event_limit)
        admitted = [row for row in events if row.get("materiality") in {"medium", "high"}][: request.event_limit]
        evidence = [_evidence(
            "event",
            f"normalized_event:{row['id']}:{row.get('detection_version')}:{_hash(row)[:16]}",
            "normalized_event_intelligence",
            as_of=row.get("occurred_at"),
            metadata={"materiality": row.get("materiality"), "source_count": len(row.get("evidence", []))},
        ) for row in admitted]
        state = ContextState.CURRENT if admitted else ContextState.MISSING
        missing = [] if admitted else [_deficiency(instrument, "relevant_events", state, "No material normalized company events are available.", {"materiality": ["medium", "high"], "limit": request.event_limit, "cadence": "event_driven"})]
        if admitted and all(row.get("freshness_status") == "stale" for row in admitted):
            state = ContextState.STALE
            missing = [_deficiency(instrument, "relevant_events", state, "All admitted events are stale under event intelligence's own policy.", {"cadence": "event_driven", "limit": request.event_limit})]
        return _section(ContextSectionName.EVENTS, state, admitted, evidence, as_of=admitted[0]["occurred_at"] if admitted else None, provenance={"bounded_limit": request.event_limit}), missing

    @staticmethod
    def _rag(db: Session, user: User, request: IntelligenceContextRequest, instrument: Instrument):
        query = request.question or (PURPOSE_QUERIES.get(request.research_purpose) if request.research_purpose else None)
        if not query:
            return _section(ContextSectionName.RAG_EVIDENCE, ContextState.NOT_REQUESTED, [], [], provenance={"reason": "no_question_or_research_purpose"}), []
        response = search_rag(db, user, RagSearchRequest(query=query, symbols=[instrument.symbol], limit=request.rag_limit))
        chunks = [chunk.model_dump() for chunk in response.chunks[: request.rag_limit]]
        evidence = [_evidence("document_passage", f"chunk:{chunk.id}:document:{chunk.document_id}", chunk.citation.source_name, as_of=chunk.citation.created_at, source_url=chunk.source_url, metadata={"citation_id": chunk.citation.id, "document_id": chunk.document_id, "page_number": chunk.page_number, "score": chunk.score}) for chunk in response.chunks[: request.rag_limit]]
