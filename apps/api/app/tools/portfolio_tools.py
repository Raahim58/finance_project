...
def _scenario_history(db, user, payload: PortfolioInput):
    return {"history": list_scenario_runs(db, user, payload.portfolio_id)}
...
def register_portfolio_tools(registry: ToolRegistry) -> None:
    ...
    registry.register(ToolDefinition("scenario.history", "1.0", "Recorded scenario runs with stressed PnL and compliance for a portfolio", PortfolioInput, "portfolio:read", True, False, 8, "low", _scenario_history))
