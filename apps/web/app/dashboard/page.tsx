...
const defaults:WidgetConfig[]=[{id:"value",size:"small"},{id:"performance",size:"medium"},{id:"allocation",size:"small"},{id:"risk",size:"medium"},{id:"holdings",size:"wide"},{id:"movers",size:"medium"},{id:"alerts",size:"medium"},{id:"macro",size:"small"},{id:"watchlist",size:"medium"},{id:"events",size:"medium"}];
...
const span:Record<Size,string>={small:"md:col-span-4 xl:col-span-3",medium:"md:col-span-6",wide:"md:col-span-12"};
...
export default function DashboardPage(){
  ...
  const hidden=defaults.filter(d=>!widgets.some(w=>w.id===d.id));
  return <div className="page-wrap">
    <header className="page-heading"><div><p className="eyebrow">Investment workspace</p><h1 className="page-title">Command dashboard</h1><p className="page-subtitle">Portfolio, market and exception signals in one configurable view.</p></div><div className="flex flex-wrap gap-2"><button className="btn btn-secondary" onClick={()=>setEditing(!editing)}><Icon name={editing?"check":"settings"}/>{editing?"Finish layout":"Customize"}</button><Link className="btn btn-primary" href="/portfolios">Review portfolios <Icon name="chevron"/></Link></div></header>
    ...
    {editing?<div className="mb-4 flex flex-wrap items-center gap-2 border border-dashed border-line-strong bg-white p-3"><span className="mr-2 text-xs font-semibold">Add widget</span>{hidden.map(w=><button key={w.id} className="btn btn-secondary" onClick={()=>save([...widgets,w])}><Icon name="plus" size={14}/>{names[w.id]}</button>)}{!hidden.length?<span className="text-xs text-muted">All widgets are visible. Use arrows to reorder and expand to resize.</span>:null}</div>:null}
    <div className="grid grid-cols-1 gap-3 md:grid-cols-12">
      {widgets.map((w,i)=><section key={w.id} className={`panel min-h-[190px] ${span[w.size]}`}><div className="panel-head"><h2 className="panel-title">{names[w.id]}</h2>{editing?<div className="flex"><button className="icon-btn" aria-label={`Move ${names[w.id]} earlier`} disabled={i===0} onClick={()=>move(i,-1)}><Icon name="arrowUp"/></button><button className="icon-btn" aria-label={`Move ${names[w.id]} later`} disabled={i===widgets.length-1} onClick={()=>move(i,1)}><Icon name="arrowDown"/></button><button className="icon-btn" aria-label={`Resize ${names[w.id]}`} onClick={()=>resize(i)} title={`Current size: ${w.size}`}><Icon name="expand"/></button><button className="icon-btn" aria-label={`Remove ${names[w.id]}`} onClick={()=>remove(i)}><Icon name="close"/></button></div>:<span className="text-[10px] uppercase tracking-wide text-muted">{data.summary?.data_freshness_date??"No date"}</span>}</div><div className="panel-body h-[calc(100%-43px)]"><Widget id={w.id} data={data} loading={loading}/></div></section>)}
    </div>
  </div>;
}
...
function Widget({id,data,loading}:{id:WidgetId;data:Data;loading:boolean}){
  ...
  if(id==="risk")return <BarChart height={205} horizontal percent labels={Object.keys(data.quant?.risk_contributions??{})} values={Object.values(data.quant?.risk_contributions??{}).map(v=>v*100)} color="#a44932"/>;
  if(id==="holdings")return s?.holdings.length?<div className="table-wrap"><table className="data-table"><thead><tr><th>Security</th><th>Sector</th><th>Market value</th><th>Day</th><th>Unrealized P&amp;L</th></tr></thead><tbody>{s.holdings.slice(0,8).map(h=><tr key={h.holding_id}><td><Link className="font-bold text-[#244e70]" href={`/companies/${h.symbol}`}>{h.symbol}</Link></td><td>{h.sector}</td><td className="data-font">{money(h.market_value)}</td><td className={Number(h.day_change_percent)>=0?"positive":"negative"}>{pct(h.day_change_percent)}</td><td className={Number(h.unrealized_gain_loss)>=0?"positive":"negative"}>{money(h.unrealized_gain_loss)}</td></tr>)}</tbody></table></div>:<Missing text="Holdings will appear after positions are added."/>;
  ...
  if(id==="alerts"){const flags=data.risk?.flags??[];return flags.length?<div className="divider-list">{flags.slice(0,5).map((f,i)=><div className="flex gap-3 py-2" key={`${f.code}-${i}`}><span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${f.severity==="high"||f.severity==="critical"?"bg-[#a44932]":"bg-[#976515]"}`}/><div><p className="text-xs font-semibold">{f.code.replaceAll("_"," ")}</p><p className="mt-1 text-[11px] text-muted">{f.message}</p></div></div>)}</div>:<div className="empty-state"><Icon name="check" size={25}/><strong>No active portfolio exceptions</strong><span>Risk checks have not raised a warning.</span></div>}
