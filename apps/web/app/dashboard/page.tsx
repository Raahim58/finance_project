...
const catalog:WidgetConfig[]=[...defaults,{id:"macro",size:"small"},{id:"watchlist",size:"medium"},{id:"events",size:"medium"}];
...
export default function DashboardPage(){
  ...
  return <div className="page-wrap">
    ...
    {!loading&&!error?<div className="source-rail mb-5 flex flex-wrap items-center justify-between gap-4 px-5 py-4"><div className="flex items-center gap-3"><span className={`grid h-8 w-8 place-items-center rounded-full ${data.risk?.flags.length?"bg-[#fbf2df] text-[#a86f18]":"bg-[#e8f3ee] text-accent"}`}><Icon name={data.risk?.flags.length?"warning":"check"} size={16}/></span><div><p className="text-[13px] font-semibold">{data.risk?.flags.length?`${data.risk.flags.length} portfolio exception${data.risk.flags.length===1?"":"s"} require review`:"No active portfolio exceptions"}</p><p className="text-[11px] text-muted">{data.summary?.portfolio.name??"No portfolio selected"}</p></div></div><p className="text-[11px] text-muted">As of {data.summary?.data_freshness_date??"unavailable"} · {data.summary?.data_source??"source unavailable"}</p></div>:null}
