...
function Response({result}:{result:AssistantResult}){return <div className="grid gap-5"><section className="source-rail px-6 py-5"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-[14px] font-semibold">Conclusion</h2><span className="text-[11px] text-muted">{new Date(result.created_at).toLocaleString()}</span></div><p className="mt-4 whitespace-pre-wrap text-[14px] leading-7">{result.answer}</p></section><Evidence title="Quantitative evidence" rows={result.calculated_evidence} empty="No calculated evidence was returned."/><Evidence title="Document evidence and sources" rows={result.source_citations} empty="No cited document evidence was returned."/>{result.uncertainty.length||result.freshness_warnings.length?<section className="panel"><div className="panel-head"><h2 className="panel-title">Uncertainty and missing data</h2><span className="badge badge-warn">Review</span></div><div className="divider-list">{[...result.uncertainty,...result.freshness_warnings].map((x,i)=><div className="flex gap-3 px-4 py-3 text-[13px]" key={i}><Icon name="warning" size={15} className="mt-0.5 shrink-0 text-[#a86f18]"/><span>{x}</span></div>)}</div></section>:null}<details className="panel"><summary className="cursor-pointer px-4 py-3 text-[12px] font-semibold">Analysis trace · {result.tool_trace.length} steps</summary><div className="divider-list border-t border-line">{result.tool_trace.map((x,i)=><div className="p-4" key={i}><p className="metric-label">Step {i+1}</p><DataRecord value={x}/></div>)}</div></details></div>}
function Evidence({title,rows,empty}:{title:string;rows:Array<Record<string,unknown>>;empty:string}){return <section className="panel"><div className="panel-head"><h2 className="panel-title">{title}</h2><span className="text-[11px] text-muted">{rows.length} item{rows.length===1?"":"s"}</span></div>{rows.length?<div className="divider-list">{rows.map((r,i)=><div className="px-5 py-4" key={i}><DataRecord value={r}/></div>)}</div>:<div className="empty-state !min-h-24">{empty}</div>}</section>}
function DataRecord({value}:{value:Record<string,unknown>}){return <dl className="grid gap-x-5 gap-y-2 sm:grid-cols-[150px_1fr]">{Object.entries(value).filter(([,v])=>v!==null&&v!==undefined).map(([k,v])=><div className="contents" key={k}><dt className="text-[11px] font-semibold capitalize text-muted">{k.replaceAll("_"," ")}</dt><dd className="m-0 break-words text-[12px] leading-5">{typeof v==="object"?Array.isArray(v)?v.map(String).join(", "):Object.entries(v as Record<string,unknown>).map(([a,b])=>`${a}: ${String(b)}`).join(" · "):String(v)}</dd></div>)}</dl>}
