...
export function DonutChart({labels,values,height=250,basis="total capital"}:{labels:string[];values:number[];height?:number;basis?:string}) {
...
export function ModelCurveChart({curve,markers=[],height=340,xName,yName,xPercent=true,yPercent=true}:{curve:Array<{x:number;y:number}>;markers?:Array<{x:number;y:number;name:string;color?:string}>;height?:number;xName:string;yName:string;xPercent?:boolean;yPercent?:boolean}) {
  if(!curve.length) return <div className="empty-state" style={{height}}><strong>Model curve unavailable</strong><span>The API did not return enough modeled points.</span></div>;
  const format=(value:number,percent:boolean)=>percent?`${(value*100).toFixed(1)}%`:value.toFixed(2);
  const colors=["#171a1d","#b9473c","#126b52"],symbols=["diamond","rect","circle"];
  return <ReactECharts notMerge lazyUpdate style={{height,width:"100%"}} option={{...base,legend:{bottom:0,textStyle:{color:muted,fontSize:11}},grid:{left:18,right:20,top:34,bottom:48,containLabel:true},tooltip:{...base.tooltip,trigger:"item",formatter:(p:{seriesName:string;data:{value:number[];name?:string}})=>`${p.data.name??p.seriesName}<br/>${xName}: ${format(p.data.value[0],xPercent)}<br/>${yName}: ${format(p.data.value[1],yPercent)}`},xAxis:{...axis,type:"value",name:xName,nameLocation:"middle",nameGap:34,axisLabel:{...axis.axisLabel,formatter:(value:number)=>format(value,xPercent)}},yAxis:{...axis,type:"value",name:yName,nameLocation:"middle",nameGap:44,axisLabel:{...axis.axisLabel,formatter:(value:number)=>format(value,yPercent)}},series:[{name:"Feasible frontier",type:"line",showSymbol:false,data:curve.map(point=>[point.x,point.y]),lineStyle:{color:"#536b70",width:2},itemStyle:{color:"#536b70"}},...markers.map((point,index)=>({name:point.name,type:"scatter",symbol:symbols[index%symbols.length],symbolSize:14,label:{show:true,position:"top",formatter:point.name,color:ink,fontSize:10},data:[{name:point.name,value:[point.x,point.y],itemStyle:{color:point.color??colors[index%colors.length],borderColor:"#fff",borderWidth:2}}]}))]}}/>;
}

export function MultiLineChart({labels,series,height=330,percent=true}:{labels:string[];series:Array<{name:string;values:Array<number|null>;color:string}>;height?:number;percent?:boolean}) {
  if(!labels.length) return <div className="empty-state" style={{height}}><strong>History unavailable</strong><span>The model did not return rolling observations.</span></div>;
  return <ReactECharts notMerge lazyUpdate style={{height,width:"100%"}} option={{...base,legend:{bottom:0,textStyle:{color:muted,fontSize:11}},grid:{left:18,right:18,top:18,bottom:48,containLabel:true},tooltip:{...base.tooltip,trigger:"axis",valueFormatter:(value:number)=>percent?`${(value*100).toFixed(2)}%`:value.toFixed(2)},xAxis:{...axis,type:"category",boundaryGap:false,data:labels,axisLabel:{...axis.axisLabel,hideOverlap:true}},yAxis:{...axis,type:"value",axisLabel:{...axis.axisLabel,formatter:percent?(value:number)=>`${(value*100).toFixed(1)}%`:(value:number)=>value.toFixed(2)}},series:series.map(item=>({name:item.name,type:"line",data:item.values,showSymbol:false,connectNulls:false,lineStyle:{width:2,color:item.color},itemStyle:{color:item.color}}))}}/>;
}
